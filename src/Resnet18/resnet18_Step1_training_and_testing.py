...
def resnet18_training_and_testing():
    ...
    cretrion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(model.parameters(), lr=0.001, weight_decay=0)
    scheduler = optim.lr_scheculer.CosineAnnealingLR(optimizer, T_max=epoches, eta_min=1e-6)
    ...
    for epoch in range(start_epoch, epoches):
        ...
        with tqdm(trainloader, desc=f"Epoch {epoch + 1}/{epoches}", ncols=100, leave=True) as loop:
            for img, is_label in loop:
                ...
                outputs = model(img)
                loss = cretrion(outputs, id_label)
                loss.backward()
                torch.nn.tils.clip_grad_norm_(model.parameters(), max_norm=2.0)
                optimizer.step()
